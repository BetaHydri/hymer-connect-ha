...
| [`mitm_hymer_ws.py`](mitm_hymer_ws.py) | mitmproxy addon — intercepts live SignalR WebSocket traffic between the EHG app and cloud, decodes PIA protobuf payloads, and logs all sensor data to JSONL files. Used during early development to reverse-engineer bus/slot mappings. | `mitmweb -s tools/mitm_hymer_ws.py --listen-port 8080` |
| [`decode_cmds.py`](decode_cmds.py) | Offline protobuf decoder — manually decode base64-encoded PIA command payloads captured from mitmproxy logs. Inspect individual SignalR messages (heater commands, light controls) at the protobuf field level. | `python tools/decode_cmds.py <base64_payload>` |
