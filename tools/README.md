...
Converts a **local** Dan-Simms EHG runtime-metadata extraction directory into a
...
### Provenance rules (agreed with Dan, 2026-05)

1. **You** lawfully obtain the EHG APK / bundle.
2. **You** run Dan's `prepare_runtime_metadata.py` locally to produce a
   metadata directory containing `sensor_labels.json`, `component_kinds.json`,
   `control_catalog.json`, `coverage_audit.json`, and (optionally)
   `support_matrix.json` / `vehicle_catalog.json`.
...
### Pin to a released tag of Dan's repo

Dan's metadata format is "stable-ish" but **not** a public API yet. Pin your
extraction to a released tag (do not run against `main`) so the field names
this converter expects remain valid. If Dan publishes a formal schema, adjust
`SCHEMA_MAP` at the top of [`convert_dan_metadata.py`](convert_dan_metadata.py)
in one place.

### Conservative emission policy

Per Dan's recommendation:
...
The metadata extraction tooling this converter consumes is the work of
**Dan Simms** (`hymer_connect_metadata` project). This repository ships only
the converter; it does not redistribute any APK-derived data.
