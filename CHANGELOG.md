...
- **Moved dev tools to `tools/`** — `decode_cmds.py` (offline protobuf decoder) and `mitm_hymer_ws.py` (mitmproxy WebSocket capture addon) relocated from `custom_components/` to `tools/` where they belong. Neither is part of the HA integration runtime. Added both to the `tools/README.md` shipped-scripts table.
...
- **`tools/convert_dan_metadata.py`** — New CLI converter that turns a *local* EHG runtime-metadata extraction directory (produced by [HYMER Connect Metadata Edition](https://github.com/dan-simms1/hymer-connect-ha) by [@dan-simms1](https://github.com/dan-simms1)) into a `sensor_maps/<brand>.json` overlay. Aimed at users whose vehicle is **not** a HYMER Grand Canyon S 600/S 700 (Bürstner, Carado, Dethleffs, Eriba variants, LMC, Laika, Niesmann+Bischoff, Sunlight, FreeOnTour, …) where `hymer.json` doesn't match. Conservative emission policy: read-only slots auto-emitted; switches require explicit `known_writable` + `control_catalog` entry; climate/fridge/boiler/heater are intentionally **not** auto-emitted (a `_climate_templates_required` marker block is written instead). Includes a built-in `self-test` subcommand. Documented in `tools/README.md`.
- **`.gitignore` hardening** — Switched `tools/` to `tools/*` and explicitly whitelisted the shipped converter + README. Added belt-and-suspenders blocks for any local extracted metadata file (`sensor_labels.json`, `component_kinds.json`, `control_catalog.json`, `coverage_audit.json`, `support_matrix.json`, `vehicle_catalog.json`) and `oauth_client.json` anywhere under `tools/**`.
...
- **Fix blocking IO warning** — Moved JSON overlay file loading to executor thread to avoid `Detected blocking call to open` warning in HA logs.
