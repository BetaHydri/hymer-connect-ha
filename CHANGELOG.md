...
- **Critical: Remove backup.py** — The backup.py added in v2.40.1 caused HA to fail with "Invalid handler specified" when adding the integration. HA cached the module failure and couldn't recover. Removed backup.py entirely.

## [2.40.2] - 2026-05-02

### Fixed

- **Critical: Remove sensor map overlay loading from startup** — The JSON overlay loading added in v2.40.0 could crash the integration if the sensor_maps directory wasn't installed correctly by HACS. Reverted to direct SENSOR_MAP usage. The overlay infrastructure remains in pia_decoder.py for future use but is not called at startup.

## [2.40.1] - 2026-05-02

### Fixed

- **HA 2026.3+ compatibility** — Added required `backup.py` platform module. Without it, HA refuses to load the integration with `ModuleNotFoundError: No module named 'custom_components.hymer_connect.backup'`.
